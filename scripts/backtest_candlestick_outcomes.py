...
def load_all_parquet_files(data_dir, universe_tickers, start_date, end_date):
    """
    Load all date-partitioned parquet files from data_dir.
    Returns a dictionary: {ticker: DataFrame} with OHLCV data filtered by date range.
    """
    data_dir = Path(data_dir)

    # Find all feature_matrix_*.parquet files
    all_files = list(data_dir.glob('feature_matrix_*.parquet'))
    ...
    logger.info(f"Total rows loaded from all files: {total_rows}")

    # Filter by universe tickers
    ...
    timestamp_cols = ['timestamp', 'date', 'Date', 'datetime']
    timestamp_col = None
    ...
    combined_df['timestamp'] = pd.to_datetime(combined_df[timestamp_col])
    ...
    # Group by ticker and sort by timestamp
    ticker_data = {}
    for ticker in universe_tickers:
        ...
        if len(ticker_df) > 0:
            ...
            logger.info(f"  Ticker {ticker}: {len(ticker_df)} rows")
        else:
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  python backtest_candlestick_outcomes.py --universe config/universe_500.csv --start 2020-01-01 --end 2026-06-17 --data_dir exports/parquet
  python backtest_candlestick_outcomes.py --universe config/universe_500.csv --start 2020-01-01 --end 2026-06-17 --limit 10
    ...
    parser.add_argument('--data_dir', type=str, default='exports/parquet',
                        help='Directory with date-partitioned parquet files (feature_matrix_YYYY-MM-DD.parquet)')
    ...
    ticker_data = load_all_parquet_files(args.data_dir, universe_tickers, start_date, end_date)

    if not ticker_data:
        logger.error("No data loaded. Check your data_dir and parquet files.")
        sys.exit(1)

    logger.info(f"Loaded data for {len(ticker_data)} tickers")

    # Database connection
    db_url = args.db_url or os.environ.get('DATABASE_URL', 'postgresql://user:password@localhost:5432/atlas_research')
