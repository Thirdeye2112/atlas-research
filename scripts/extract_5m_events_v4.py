...
Output: reports/validity/smc_events_v4.parquet
...
OUT=ROOT/"reports"/"validity"/"smc_events_v4.parquet"
...
SCHEMA=pa.schema([
    ...
    ("max_fav_R",pa.float64()),("stopped",pa.bool_()),("mtm_R",pa.float64()),("mae_R",pa.float64()),("risk_pct",pa.float64()),
...
def main():
    ...
    print(f"extract v4 | {len(univ):,} tickers -> {OUT}")
        ...
        with connection.get_connection() as conn:
            for tk in univ:
                ...
                for i in range(2*K,n-1):
                    ...
                    entry=o[i+1]; swing=np.min(l[i-K+1:i+1])
                    atrfloor=0.5*atr5[i] if np.isfinite(atr5[i]) else 0.0
                    stop_dist=max(entry-swing, atrfloor, MIN_STOP_PCT*entry)
                    ...
                    rows.append((tk,ts_et.iloc[i],bool(ts_et.iloc[i]>=OOS_START),*flags,
                                 sim[0],sim[1],mtm,sim[2],float(stop_dist/entry),sim[3],sim[4]))
                if rows:
                    cols=list(zip(*rows))
